...
export default function Slider({ images }: SliderProps) {
  ...
  useGSAP(() => {
    ...
    tl.to(cards, {
      rotate: (index) => itemValuesRef.current[index % cardsLength] ?? 0,
      xPercent: (index) => itemValuesRef.current[index % cardsLength] ?? 0,
      yPercent: (index) => itemValuesRef.current[index % cardsLength] ?? 0,
      scale: 0.95,
      duration: 0.5,
      ease: "back.inOut(3)",
    });
      ...
      >
        {allImages.map((imageUrl, index) => (
          <motion.div
            ...
            className="slider-card p-3 rounded-xl bg-[#cfc6eb]"
          >
            <div className="h-[clamp(260px,35vw,400px)] w-[clamp(320px,40vw,450px)] rounded-lg overflow-hidden relative select-none">
              <Image
                src={imageUrl}
                ...
                className="object-cover object-center pointer-events-none select-none h-full"
              />
            </div>
          </motion.div>
        ))}
